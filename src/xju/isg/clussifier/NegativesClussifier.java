...
public class NegativesClussifier {
	...
	private static String log_but = "log/log_err_but_noSpace3.txt";
	private static String log_mainPart = "log/log_err_mainPart.txt";
	
	public static void main(String[] args) 
	{ 
		Initialize(); 
		getButModelEmo((float)0.43, 1, 1);  //  2 = 三个窗口  
		
//		for(float i = 0f; i <= 2.1f; i = i + 0.1f)
//		{
//			getButModelEmo((float)0.43, 1, 1, i); 
//		}
		
		//  (4) 否定词       max : stemWeith = 0.43~0.45    degSpace = 1     noSpace = 0        0.7275014
//		PrintWriter pwWeight = FileRW.setWriter("log/log_weight_NoSpace_news.txt", encoding); 
...
//		SOP("Big = "+big); 
	...
	{ 
		PrintWriter pw = FileRW.setWriter("log/MovieResult/rang35/no_rang35_train.txt", encoding); 
		float rang = (float)3.5; 
		int count = 0; 
		int allCount = 0; 
...
//		PrintWriter  pwBut = FileRW.setWriter(log_but, encoding);  // 创建 basic 的 log 文件 
		...
		{ 
			...
			boolean hasAbsouteWord = false; 
			hasAbsouteWord = getAbsoluteNegSent(entry.toString()); 
...
//					SOP(threeElimentSet[i]); 
				...
				if (acc >= 0 && butLog.split("\t\t").length > 2) 
					acc = ((float)(-1.0) * acc) + (float)(-0.1);
				
				if (hasAbsouteWord && acc >= 0)
					acc = acc * (float)(-1);
				...
				if(bool == 1)
				{
					...
					{
						count++;
//						pw.println("Pos情感分数 = " + String.valueOf(acc) + "\t" + entry.getKey()); 
					}
					a = a + 1;
				}
				else if(bool == 2)
...
//						pwBut.println("否定词 = " + butLog); 
				...
				else if(bool == 4)
				{
					...
					{
						count++; 
//						pw.println("Neg情感分数 = " + String.valueOf(acc) + "\t" + entry.getKey()); 
					}
					d = d + 1;
				}
				else {
...
//				if(acc >= (-1)*rang && acc <= rang) 
//				{
//					pw.println("情感分数 = " + String.valueOf(entry.getValue()) + "\t" + entry.getKey()); 
//				}
...
//		pwBut.close();
		pw.close();
//		SOP("HashMap = " + sentHashMap.size() + "\t\tA+B+C+D = " + (a + b + c + d));
		SOP(rang + "=" + allCount + "=" + count + "=" + ((float)count * 100f / (float)allCount) + "=" + ((float)allCount * 100f / (a + b + c + d)));
		return Accuracy;
	} 
	
	private static boolean getAbsoluteNegSent(String sent)
	{
		if(sent.contains("ئېسىت") || 
				sent.contains("ئىسىت") 
				|| sent.contains("ئاپلا") 
				|| sent.contains("خەپ") 
				|| sent.contains("ئەستاخپىرۇللا") 
				|| sent.contains("نىمىشقىمۇ")
				|| sent.contains("ئەپسۇس") 
//				|| sent.contains("ئەستا")
				)
			return true;
		return false;
	...
	{															  //   0	   1        2         3        4         5        6         7
		...
		for (int i = 0; i < posWords.size(); i++)  //  检查 正面词
		{
			...
			{
				for (int b = index, butLock = 1; (b + 1) < ft.length && butLock <= noSpace; b++, butLock++)  // 判断 是否有 否定词 
				{
					String Origin = ft[b + 1].split("=")[0]; 
					String Property = ft[b + 1].split("=")[1]; 
					String Stem = ft[b + 1].split("=")[2]; 
	...
	{
		HashSet<String> butWords = new HashSet<String>();
		butWords.add("ئەمەس");
//		butWords.add("يوق");  // 没有变化
		String butWord2 = "قالما";
		String[] butSuffx = {"مى",
							 "مە",   // 准确率 下降
							 "ما",   // 准确率 下降
							 };
		
		if(Origin.contains(butWord2)) 
			return null; 
		
		if((butWords.contains(Origin) && !Origin.equals("ئەمەسمۇ"))) 
			return new String[]{"1",Origin}; 
		else if(Property.equals("V")) 
			for (int i = 0; i < butSuffx.length; i++) 
				if(suffx.contains(butSuffx[i]) 
						&& !suffx.contains("مىدۇ") 
						&& !suffx.contains("مىز") 
						&& !suffx.contains("مەن") 
						&& !suffx.contains("ماق") 
						&& !suffx.contains("مەك") 
						&& !suffx.contains("مىكى")  // 新添加的
						&& !suffx.contains("مامدۇ")  // 新添加的
						&& !suffx.contains("مەمدۇ")  // 新添加的
						&& !suffx.contains("مامسەن")  // 新添加的
						&& !suffx.contains("تىما")  // 新添加的
						&& !suffx.contains("دىكەن")  // 新添加的
						&& !suffx.contains("تىكەن")  // 新添加的
						&& !suffx.contains("مىكەن")  // 新添加的
						&& !suffx.contains("دەك")  // 新添加的
						&& !suffx.contains("پتىمە")  // 新添加的
						&& !suffx.contains("مىغاي")  // 新添加的
//						&& !suffx.contains("مىگەن")  // 新添加的  准确率 下降
//						&& !suffx.contains("مىسى")  // 新添加的   准确率 下降
//						&& !suffx.contains("غىيمىدى")  // 新添加的  准确率 下降
//						&& !suffx.contains("ماسمىدى")  // 新添加的  准确率 下降
						)
